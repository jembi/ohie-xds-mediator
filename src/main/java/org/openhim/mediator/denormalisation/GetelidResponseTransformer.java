...
public class GetelidResponseTransformer extends AbstractMessageTransformer {
	...
	public Object transformMessage(MuleMessage message, String outputEncoding)
		...
		if (entityID.startsWith("urn:uuid:")) {
			...
			resultMap.put(Constants.ELID_MAP_ID, entityID);
			resultMap.put(Constants.ELID_AUTHORITY_MAP_ID, Constants.UUID_OID_AUTHORITY);
		} else if (entityID.startsWith("urn:oid:")) {
			entityID = entityID.replace("urn:oid:", "");
			resultMap.put(Constants.ELID_MAP_ID, entityID.substring(entityID.lastIndexOf('.') + 1));
			resultMap.put(Constants.ELID_AUTHORITY_MAP_ID, entityID.substring(0, entityID.lastIndexOf('.')));
		} else {
